# HakunaMatata — Security Researcher

## About Me

I started programming smart contracts in 2022 and worked for two years as a Smart Contract Developer. Since February 2025, I’ve been focusing on auditing smart contracts on platforms like Sherlock, Code4rena and CodeHawks.

- [Code4rena Profile](https://code4rena.com/@hakunamatata)
- [Sherlock Profile](https://audits.sherlock.xyz/watson/BusinessShotgun)

🏆 Achievements: **12 payouts • 3×1st place • 7×Top10 finishes**

## Public Audits

| Project                         | Platform  | Placement | Protocol Type                   | Language |
| ------------------------------- | --------- | --------- | ------------------------------- | -------- |
| **StarkWare Perps**             | Code4rena | 🥇 1st    | Perpetual Exchange              | Cairo    |
| **Blackhole**                   | Code4rena | 🥇 1st    | ve(3,3)                         | Solidity |
| **Neutrl Protocol**             | Sherlock  | 🥇 1st    | Stablecoin                      | Solidity |
| **Ekubo**                       | Code4rena | 🥉 3rd    | CLAMM (uni v4 fork)             | Solidity |
| **Aave DIVA Wrapper**           | CodeHawks | 🥉 3rd    | Aave Integration                | Solidity |
| **Rova**                        | Sherlock  | 🥉 3rd    | Launchpad                       | Solidity |
| **Nudge.xyz**                   | Code4rena | 5th       | Liquidity Reallocation Protocol | Solidity |
| **Summer.fi Governance V2**     | Sherlock  | 5th       | Cross-chain Governance          | Solidity |
| **IQ, AI**                      | Code4rena | 13th      | Governance                      | Solidity |
| **Symmio, Staking and Vesting** | Sherlock  | 16th      | Staking                         | Solidity |
| **Vultisig**                    | Code4rena | 30th      | Launchpad                       | Solidity |
| **Malda**                       | Sherlock  | 43rd      | Cross-Chain Lending             | Solidity |
| **Trait Forge**                 | Code4rena | 78th      | NFT game                        | Solidity |

## Private Audits

| Project       | Protocol Type | Language |
| ------------- | ------------- | -------- |
| **FixedLend** | P2P Lending   | Cairo    |

---
